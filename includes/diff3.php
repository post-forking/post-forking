<?php
/**
 * Three way diff engine that extends WordPress's existing diff engine
 * Used under the LGPL
 * Source: http://pear.php.net/package/Text_Diff/docs/latest/Text_Diff/Text_Diff3.html
 * @package fork
 */
...
class Text_Diff3 extends Text_Diff {
	...
	function Text_Diff3( $orig, $final1, $final2 ) {
		self::__construct( $orig, $final1, $final2 );
	}
	...
	function _diff3($edits1, $edits2)
	{
		...
		while ($e1 || $e2) {
			if ($e1 && $e2 && is_a($e1, 'Text_Diff_Op_copy') && is_a($e2, 'Text_Diff_Op_copy')) {
				...
				if ($e1 && $e2) {
						$norig = 0; //added to prevent undefined errors below
					if ($e1->orig && $e2->orig) {
